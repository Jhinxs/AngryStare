...
namespace CallWinProCallBacK
{
    class PEB
    {
        public PEB(string binpath)
        {

            MasqueradePEB(binpath);
        ...
        public static bool MasqueradePEB(string masqBinary)
        {
            ...
            //Masquerade windows title
            IntPtr WindowsTitle = IntPtr.Zero;
            WindowsTitle = new IntPtr(peb.ProcessParameters.ToInt64() + 0xb0);
            if (!RtlInitUnicodeString(procHandle, WindowsTitle, "WindowTitle", masqBinary))
            {
                return false;
            }
